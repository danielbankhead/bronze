...
function displaySpecs () {
  console.log(Object.keys(Bronze.specs).join(' '))
}
function displayVersion () {
  console.log(packageInfo.version)
}

function updateSequenceFile (sequencePath, sequenceCount) {
...
function readFromSequenceFile (sequencePath) {
  function makeIfDirDoesntExist (givenPath) {
    const baseDir = path.parse(givenPath).dir
    ...
    try {
      ...
      if (e.code === 'EEXIST') {
        return baseDir
      } else if (e.code === 'ENOENT' && baseDir !== givenPath) {
        makeIfDirDoesntExist(baseDir)
      } else {
        throw e
      }
    }
  ...
  try {
    ...
    if (e.code === 'ENOENT') {
      makeIfDirDoesntExist(sequencePath)
    } else {
      throw e
    }
...
function generateIds (options, count, sequencePath) {
  const idGen = new Bronze(options)

  for (let i = 0; i < count; i++) {
    console.log(idGen.generate())
  }
...
function parseCommand () {
  ...
  for (let i = 0, len = givenArgs.length; i < len; i++) {
    ...
    } else if (arg.indexOf('--sequence-reset') === 0) {
  ...
  if (isHelp === true) {
    ...
    if (resetSequenceFile === true) {
      try {
        updateSequenceFile(sequencePath, 0)
      } catch (e) {
        if (e.code !== 'ENOENT') {
          throw e
        }
    ...
    if (typeof bronzeOptions.sequence === 'number') {
      generateIds(bronzeOptions, generateIdCount)
    } else {
