...
function makeNestedDirectory (givenPath = '') {
  if (fs.existsSync(givenPath) === true || givenPath === '') return
  ...
  try {
    ...
    if (e.code === 'ENOENT' && givenPath !== candidatePathDir) {
...
function parseCommand (givenArgs = process.argv.slice(2)) {
  ...
  for (let i = 0, len = givenArgs.length; i < len; i++) {
    ...
    if (arg.indexOf('--pid') === 0) {
      ...
      if (typeof valueCandidate === 'string') {
        sequencePath = path._makeLong(path.join(path.resolve(valueCandidate), 'sequence'))
        if (advanceAdditionalCandidate === true) {
          advanceAdditional = true
        }
      }
